...
import "styles/PinnedMessage.scss";
    ...
    if (post) {
      switch (post.meta.type) {
        ...
        case 'pin':
          ...
          content = <div>PINNED: {JSON.stringify(post, null, 2)}</div>
          break;
        case 'file':
      ...
      <div className={className} style={style} onDragEnter={onDragEnter}>
        <div className="row2">
          <div className="Body">
            ...
            {user && this.state.post && this.props.currentUserId !== user.id
              ? <span>
                  <span className="spacer">  |  </span>
                  <div className="ActionButton" onClick={this.onPin.bind(this)}>Pin</div>
                </span>
              : null
            }
          </div>
